...
We will start with the same chart we created in R, a **scatterplot**. For this example I make use of the *pipe* command, |>. This tells R puts whatever is one the left of the *pipe* into the first place in the command to the right of the \em{pipe}. The command for generating the chart is **ggplot**.
...
$$ggplot(data=nfldata,aes(x=PF,y=PA)) + geom_point()$$
...
```
I am not a fan of the scientific notation on the y-axis. So let's change that. 
